...
/* Phone-width tune — shrink image column + drop type one notch. */
...
Pair that with a responsive `BANNER_WIDTH` on the `AdSlot` itself — `min(520px, calc(100vw - 48px))` — and the slot is well-behaved from a 360px phone all the way up to a wide desktop.
