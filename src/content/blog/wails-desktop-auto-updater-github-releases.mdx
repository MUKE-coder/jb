...
**Last updated: June 2026 · By [JB (Muke Johnbaptist)](/) — this is the exact auto-updater shipped in [Shoppleet](https://shoppleet.com) v1.5.85+. Hundreds of branch installs, six months unattended, zero update-related support tickets.**
...
After this, `wails build -nsis` produces both `MyApp-v1.2.3.exe` (the raw, swappable binary) and `MyApp-Setup-v1.2.3.exe` (the wizard). The release script ships both.
...
- **"Minimum supported version" gate.** Return `min_supported_version` from `/api/desktop/latest`. If `APP_VERSION` is below it, refuse to start the app and force the update. We use this to retire legacy clients that depend on now-broken API contracts.
...
_This article is based on the Shoppleet desktop auto-updater shipped in v1.5.85 and improved through v1.5.93. The pattern has run unattended across hundreds of installs for six months without an update-related support ticket._
