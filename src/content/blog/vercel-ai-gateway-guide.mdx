...
description: "Step-by-step guide to setting up the Vercel AI Gateway and using one API key to access Anthropic Claude for text reasoning, Gemini for image and vision, and OpenAI nano models for cost-sensitive routes. Covers API key creation, streaming, tool calling with custom UI, automatic fallbacks, cost tracking, and the exact provider mix JB uses in production."
image: https://14j7oh8kso.ufs.sh/f/HLxTbDBCDLwf3XoMbGWIIoG6DYfZiHgUkB8udwC12QVF4TNe
category: tutorial
...
### Real-world tool calling: a shopping assistant with custom UI

Here's the same pattern but doing actual work — a small e-commerce assistant that registers users, finds products, and processes orders. Three tools, all backed by Prisma, all called by the model autonomously:
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    model: "anthropic/claude-sonnet-4.6",
    system: `You are a shopping assistant. Always greet the user, then help them
browse categories, pick products, and check out. Never invent products —
always read from tool output. After every tool call, write one short sentence.`,
    ...
    tools: {
      ...
      searchProducts: tool({
        description: "Search products by category slug or free-text query.",
        parameters: z.object({
          categorySlug: z.string().optional(),
          query: z.string().optional(),
        }),
        execute: async ({ categorySlug, query }) => {
      ...
      processOrder: tool({
        description: "Place the order after delivery address is confirmed.",
        parameters: z.object({
          items: z.array(
        ...
        execute: async ({ items, address }) => {
          const order = await prisma.order.create({
            data: { address, items: { create: items } },
          });
          // …stripe payment intent, resend confirmation email…
          return { orderId: order.id, status: "PLACED" };
        },
      }),
    },
    stopWhen: "tool-and-text",
  });
...
On the client you don't just render text — you render **custom React components for each tool**. The AI SDK exposes `message.parts`, and your switch statement maps `tool-{name}` to the right component:
      ...
      {m.parts.map((part, i) => {
        switch (part.type) {
          ...
          case "tool-processOrder":
            return part.state === "output-available" ? (
              <OrderReceipt key={i} data={part.output} />
            ) : null;
        }
      })}
    </div>
...
Now the model decides _which tool_ to call. Your UI decides _how to render_ the result. The chat is part text, part interactive cards — exactly how WhatsApp Business catalogs feel.

> 📚 **Full deep-dive on this pattern:** [AI Tool Calling with Custom UI Components](/blog/ai-tool-calling-custom-ui) — full repo walkthrough with the 5-tool chat-shop reference, Zustand cart, Stripe checkout, and the production patterns I always apply.

### Same tools, different surfaces

The beauty of this pattern is the same tools can be called from totally different surfaces:

- Your own React chat UI (as above)
- An MCP server exposed to Claude Desktop, Cursor, Windsurf — see [From CRUD to MCP](/blog/build-mcp-server-from-crud)
- A RAG-backed agent that uses retrieval _and_ tools — see [Building a multi-tenant RAG agent platform](/blog/rag-agent-platform)
- A multi-stage content pipeline where each step calls a tool then hands off — see [Multi-model AI content pipelines](/blog/multi-model-ai-content-pipeline)

One tool definition, infinite frontends.
...
### Real-world use cases built on this Gateway

The four guides below are deep dives into production apps I've shipped on top of this exact Gateway setup — each one shows a different shape of AI feature:

- **[AI Tool Calling with Custom UI Components](/blog/ai-tool-calling-custom-ui)** — turn the model into an agent that renders product cards, address forms, and order receipts inline in chat. Reference: [chat-shop](https://github.com/MUKE-coder/chat-shop).
- **[From CRUD to MCP Server](/blog/build-mcp-server-from-crud)** — expose your existing Next.js + Prisma app to Claude Desktop, Cursor, and Windsurf via the Model Context Protocol. Reference: [contact-mcp](https://github.com/MUKE-coder/contact-mcp).
- **[Building a Multi-Tenant RAG Agent Platform](/blog/rag-agent-platform)** — ingest PDFs/DOCX/URLs, embed with `text-embedding-3-small`, stream grounded answers via SSE. Go + Qdrant + Asynq. Reference: [nexora](https://github.com/MUKE-coder/nexora).
- **[Multi-Model AI Content Pipelines](/blog/multi-model-ai-content-pipeline)** — Claude drafts image prompts, Gemini renders them, Gemini Flash returns structured JSON. Fire-and-forget 11-slide carousel generation. Reference: [jbai](https://github.com/MUKE-coder/jbai).

### Other related guides

- [Complete Guide to AI Integration with Vercel AI SDK](/blog/ai-integration-guide)
- [Building an AI-Powered E-commerce Chatbot](/blog/ai-shop-assistant)
- [Building a Production-Ready MCP Server (beginner-friendly walkthrough)](/blog/building-mcp-server-contacts)
