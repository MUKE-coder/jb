---
title: "Securing Your First VPS (and Installing Dokploy) — A Total Beginner's Guide"
description: "A no-jargon walkthrough for hardening a fresh Contabo / Hetzner / DigitalOcean VPS and optionally installing Dokploy. Uses a single script that locks down SSH, sets the firewall, installs Fail2ban, and (if you don't have one) generates an SSH key for you. Ends with a security score out of 100 and a full emergency playbook for when things go wrong."
image: https://14j7oh8kso.ufs.sh/f/HLxTbDBCDLwfKgQiRVMqtMYEbJOpeUSCcaQNrA3Lh9kd7s64
category: tutorial
...
**Last updated: May 2026 · By [JB (Muke Johnbaptist)](/) — the [vps-harden script](https://github.com/MUKE-coder/vps-harden) is open source and the same one I run on every new server.**
...
**The script runs on the SERVER, not your laptop.** You connect _from_ your laptop _to_ the server, then run the script there.

Picture a phone call: your laptop is _you_, the server is the _person you called_. The work happens on their end — you're just talking to them through the call. Throughout this guide, each step is tagged 💻 (do this on your laptop) or ☁️ (do this on the server) so you're never unsure.
...
If you left the key blank, the script created a fresh key pair _on the server_ and printed instructions. The **private key** is now sitting on the server, and you need to bring it down to your laptop — it's your only way to log back in.
...
- ❌ **If it fails:** stay calm and switch back to your **still-open** original window. You never lost access. Investigate, or just re-run the script. _This is exactly why we kept it open._
...
(Drop the `-i ~/.ssh/deploy_key` part if you used your own key.) Leave that window running, then open **http://localhost:3000** in your laptop's browser. The dashboard appears, but only _you_ can reach it because it travels through your secure connection. Create your admin account (first signup becomes admin) and enable two-factor authentication.
...
`--audit-only` means it _only looks_ and changes nothing. You'll see:
...
**Your rescue door is your provider's web console.** Contabo, Hetzner, DigitalOcean, etc. all give you a browser-based terminal that connects to the server screen _directly_, bypassing SSH entirely. In Contabo it's under your **VPS control panel → "VNC / Web Console" (or "Rescue")**. Log in there with your **root password**.
...
| `git pull`                          | Get the newest version later (run inside `vps-harden` folder) |
...
- [The Complete Linux Server Security Guide — SSH Keys, Fail2Ban & Beyond](/blog/ssh-key-security-guide) — the deeper story behind why every step in this script exists, written from a real Contabo compromise + recovery.
- [Self-Hosting Next.js Apps with Hetzner and Dokploy](/blog/self-deploy-nextjs-using-dokploy-hetzner-and-github) — once your VPS is locked down, this is the push-to-deploy pipeline I use on top of it.
- [Deploying Next.js with Coolify and Hetzner](/blog/deploy-nextjs-using-coolify-and-hezner) — same shape, different PaaS, if you prefer Coolify.
- [Migrating a Postgres database to Dokploy](/blog/migrate-postgres-to-dokploy) — for when you're ready to move stateful workloads onto the same box.
- [Docker + Postgres + pgAdmin setup](/blog/docker-postgres-pgadmin-setup) — the next thing to install once the host is secure.

---

## Need help locking down your VPS?

I run this exact playbook on every server I deploy, including Shoppleet, DGateway and Desishub client infrastructure.

- 📞 **[Book a session](/book)** — 1-on-1 setup, audit, or recovery. Sessions from UGX 50,000.
- 💼 **Hire Desishub** for full DevOps + deployment — [desishub.com](https://desishub.com)
- 📺 **YouTube** — practical Linux + Dokploy tutorials at [@JBWEBDEVELOPER](https://www.youtube.com/@JBWEBDEVELOPER)
- 💻 **Source / issues**: [github.com/MUKE-coder/vps-harden](https://github.com/MUKE-coder/vps-harden)
...
- **Script repo**: [github.com/MUKE-coder/vps-harden](https://github.com/MUKE-coder/vps-harden)
- **Dokploy**: [dokploy.com](https://dokploy.com)
- **Contabo VPS**: [contabo.com](https://contabo.com)
- **Hetzner Cloud**: [hetzner.com/cloud](https://www.hetzner.com/cloud)
- **OpenSSH manual**: [man.openbsd.org/ssh](https://man.openbsd.org/ssh)
- **Fail2ban docs**: [fail2ban.org](https://www.fail2ban.org)
